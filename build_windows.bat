...
rem    DS5_LOG_LEVEL   0..3 (default 0 for release)
rem    DS5_BUILD_PROFILE standard (default) | diagnostic
rem ============================================================
...
if "%DS5_LOG_LEVEL%"=="" set "DS5_LOG_LEVEL=0"
if "%DS5_BUILD_PROFILE%"=="" set "DS5_BUILD_PROFILE=standard"
...
:build_one
    ...
    if /i not "%DS5_BUILD_PROFILE%"=="standard" if /i not "%DS5_BUILD_PROFILE%"=="diagnostic" (
        echo [ERROR] Invalid DS5_BUILD_PROFILE "%DS5_BUILD_PROFILE%". Use standard or diagnostic.
        exit /b 2
    )
    rem ---- auto clean when board/usb/log config changes ----
    set "BUILD_KEY=%BOARD_TYPE%-%USB_SPEED%-%DS5_BUILD_PROFILE%-log%DS5_LOG_LEVEL%"
    ...
    echo [build] Target: %BOARD_TYPE%  USB: %USB_SPEED%  PROFILE: %DS5_BUILD_PROFILE%  LOG: %DS5_LOG_LEVEL%
    ...
    set "PROFILE_SUFFIX="
    if /i "%DS5_BUILD_PROFILE%"=="diagnostic" set "PROFILE_SUFFIX=-diag"
    copy /y "build\build_out\ds5dongle_bl618_bl616.bin" "%OUT_DIR%\ds5dongle-%BOARD_TYPE%%SPEED_SUFFIX%!PROFILE_SUFFIX!.bin" >nul
    copy /y "build\build_out\boot2_bl616_isp_release_v8.1.8.bin" "%OUT_DIR%\" >nul
    copy /y "build\build_out\partition.bin" "%OUT_DIR%\" >nul
    echo [build] Output: %OUT_DIR%\ds5dongle-%BOARD_TYPE%%SPEED_SUFFIX%!PROFILE_SUFFIX!.bin
    goto :eof
