...
Windows-native GUI/CLI for DS5DONGLE-AIM61. Version 1.4.0 provides a white high-contrast interface with four tabs:
...
4. Firmware flasher for verified GitHub Release ZIPs, local packages, CH340 recovery, signed Standard/Diagnostic OTA switching, and manually selected signed OTA ZIPs.

The default online list shows the newest AIM61 High-Speed Standard firmware only. Diagnostic firmware is exposed through the advanced control. The tool validates GitHub asset digests, package checksums and target metadata. Local UART packages also require `SHA256SUMS.txt`. For OTA, the host verifies the exact canonical P-256 signature before transfer, the device independently verifies it again, and the tool waits for USB re-enumeration and matching `0xF8` identity before reporting success. Diagnostic OTA additionally requires a valid `0xFD` snapshot.
