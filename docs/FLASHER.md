# DS5Dongle Windows 工具中心 v1.3.1

`DS5Dongle-Flasher-Windows-v1.3.1.exe` 是 DS5DONGLE-AIM61 的单文件 Windows 原生工具。公开配置网站已下线，仓库不再包含配置网页代码；测试、诊断、OTA 配置切换和 UART 恢复都在本机完成，诊断数据不会自动上传。
...
- 生成灯条/玩家灯/静音灯、限强度震动和自适应扳机测试信号；
- 使用与 `ds.evua.cc` 公开实现一致的 DS5 HID Output/Feature 报告测试声音、震动和扳机；
- 通过 Windows UAC 端点录制并回放 M61 麦克风，验证真实输入链路，而不只是切换静音灯；
...
默认在线列表只显示最新 **AIM61 High-Speed Standard（常用版）**。勾选“高级固件（显示诊断版）”后才显示 Diagnostic ZIP。也可选择本地完整 ZIP 或解包目录。
...
- Standard 不枚举 `0xFD`、不创建诊断任务，也不采集内部桥接计时，适合日常最低开销运行；
- Diagnostic 枚举 `0xFD`，仅在主机显式开启会话时每秒生成快照；
- “进入诊断模式（OTA）”和“恢复常用版（OTA）”使用 Release ZIP 内的签名 `.bin.ota` 与清单；切换前自动停止所有测试输出；
- 同版本 `standard ↔ diagnostic` 允许切换。设备校验板型、HS、版本策略、正文 SHA-256 和 P-256 签名后才切换 A/B 槽。
...
`0xF8` Feature Report 返回 `版本|配置`，例如 `3.5.2|standard`。Standard 与 Diagnostic 都支持读取身份；只有 Diagnostic 支持 `0xFD` v2 快照。旧固件返回单独版本或 v1 快照时，工具按兼容模式显示，缺失字段标记为“不支持”，不会伪造为零。

CLI：

```powershell
.\DS5Dongle-Flasher-Windows-v1.3.1.exe --device-info
.\DS5Dongle-Flasher-Windows-v1.3.1.exe --diagnostics
.\DS5Dongle-Flasher-Windows-v1.3.1.exe --list
.\DS5Dongle-Flasher-Windows-v1.3.1.exe --list-releases --board aim61 --usb-speed hs
.\DS5Dongle-Flasher-Windows-v1.3.1.exe --verify-release --release v3.5.2 --board aim61 --usb-speed hs
.\DS5Dongle-Flasher-Windows-v1.3.1.exe --release v3.5.2 --board aim61 --usb-speed hs --port COM3 --dry-run
```
...
python tools\package_firmware.py --board aim61 --usb-speed hs `
  --profile standard --version 3.5.2 `
  --firmware-dir firmware\aim61 --output-dir dist

python tools\package_firmware.py --board aim61 --usb-speed hs `
  --profile diagnostic --version 3.5.2 `
  --firmware-dir firmware\aim61 --output-dir dist
```
